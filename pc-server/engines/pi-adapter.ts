...
//
// runtime 透传:matches() 已算出 WorkspaceRuntime,run() 还需要它(cwd/root 装配资源)。
// 为保持「判定只在编排器入口发生一次」的语义,用模块级 WeakMap 把 matches 的 runtime 结果
// 带给同一会话的 run()——run 内重新调用 workspaceRuntimeForConversation 兜底(防御弱引用
// 被回收的极端情况),双重来源取同一判定函数,结果天然一致。
...
import type { EngineAdapter, EngineRunContext, PiRunFn } from "./index";

export function createPiAdapter(piRun: PiRunFn): EngineAdapter {
  ...
  return {
    ...
    run(ctx, sink, signal) {
      const runtime = runtimeByConversation.get(ctx.conversation) ?? workspaceRuntimeForConversation(ctx.conversation);
      const withRuntime: EngineRunContext & { piRuntime: WorkspaceRuntime | null } = { ...ctx, piRuntime: runtime };
      return piRun(withRuntime, sink, signal);
    },
  };
}
